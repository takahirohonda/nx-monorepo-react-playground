...
const bigCommerceToVercelCartItems = (
  ...
  const getItemMapper = (products: ProductsList, isCustomItem = false) => {
    return (item: CartCustomItem | DigitalOrPhysicalItem): VercelCartItem => {
      ...
      if (isCustomItem) {
        ...
        const productData = products.filter(
          ({ productId }) =>
            productId === (item as DigitalOrPhysicalItem).productEntityId
        )[0]?.productData

        product = productData
          ? bigCommerceToVercelProduct(productData)
          : vercelProductFallback
        selectedOptions = (item as DigitalOrPhysicalItem).selectedOptions.map(
          (option) => ({
            name: option.name,
            value:
              option.value ||
